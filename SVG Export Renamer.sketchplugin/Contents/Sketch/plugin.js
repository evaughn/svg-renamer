...
/*!***********************!*\
  !*** ./src/dialog.js ***!
  \***********************/
/*! exports provided: suiteName, showDialog, getDefaults, savePreferences, resetPreferences */
...
function createPrefixView(parentViewWidth, parentViewHeight) {
  ...
  var _getDefaults = getDefaults(),
      userUsePrefix = _getDefaults.usePrefix,
      userUseDefault = _getDefaults.useDefaultPrefix;

  var baseY = parentViewHeight - 25;
...
function createSuffixView(parentViewWidth, parentViewHeight) {
  ...
  var _getDefaults2 = getDefaults(),
      userUseSuffix = _getDefaults2.useCustomSuffix;
...
function configureName(name) {
  ...
  if (usePrefix) {
    if (pluginUseDefaultPrefix) {
      name = "".concat(defaultPrefix, "-").concat(name);
    }

    if (pluginUseCustomPrefix) {
      name = "".concat(customPrefix, "-").concat(name);
    }
  }

  if (useCustomSuffix) {
    name += "-".concat(customSuffix);
  }
  ...
  if (filesToRename.length > 0) {
    ...
    var oldFilePaths = filesToRename.reduce(function (dictionary, fileDict) {
      ...
      if (nameArray.length === 1 || typeName === "default" || !!parseInt(typeName)) {
        ...
        exportName = configureName("".concat(categoryName, "-").concat(typeName));
      }
